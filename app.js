// app.js - Enhanced library application with authentication integration
import './auth.js';

// Configuration and State (from previous version)
...
function initFuse() {
  ...
  state.fuse = new Fuse(state.books, {
    keys: ['title', 'author', 'field', 'tags', 'description', 'filename'],
...
// Manifest Loading with Library Mode Support
async function tryLoadManifest() {
  try {
    const mode = window.libraryMode || 'lite';
    const config = LIBRARY_CONFIGS[mode];
...
// Enhanced scanBooks with library mode support
async function scanBooks() {
  ...
  // Process books with correct base URLs
  state.books = manifest.map(item => ({
    ...
    path: item.path.startsWith('http') ? item.path : config.baseUrl + item.path,
...
function exportBooksJson() {
  const manifest = state.books.map(b => {
    // Remove base URL for export to keep paths relative
    const mode = window.libraryMode || 'lite';
    const config = LIBRARY_CONFIGS[mode];
    let path = b.path;
  ...
  reader.onload = (e) => {
    try {
      ...
      state.books = arr.map(enrich => ({
        ...
        path: enrich.path.startsWith('http') ? enrich.path : config.baseUrl + enrich.path,
      ...
      rebuildFacets();
      applyFiltersAndRender();
      alert('Imported books.json successfully.');
    } catch (err) {
...
function bindLibraryEvents() {
  ...
  $('#prevPageBtn').addEventListener('click', async () => { 
    if (state.pdf.pageNum > 1) { state.pdf.pageNum--; await renderPdfPage(); } 
  });
  $('#nextPageBtn').addEventListener('click', async () => { 
    if (state.pdf.pageNum < state.pdf.pageCount) { state.pdf.pageNum++; await renderPdfPage(); } 
  });
  $('#zoomInBtn').addEventListener('click', async () => { 
    state.pdf.scale = Math.min(state.pdf.scale + state.pdf.step, state.pdf.maxScale); 
    await renderPdfPage(); 
  });
  $('#zoomOutBtn').addEventListener('click', async () => { 
    state.pdf.scale = Math.max(state.pdf.scale - state.pdf.step, state.pdf.minScale); 
    await renderPdfPage(); 
  });
...
// Main Library App Initialization
...
window.initializeLibraryApp = initializeLibraryApp;

// Initialize if already on library page (for development)
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', () => {
    if (document.getElementById('libraryApp').style.display !== 'none') {
